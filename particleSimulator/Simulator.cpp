...
Simulator::Simulator(sf::Vector2u window_size) : width((window_size.x - 40) / 4), height((window_size.y - 140) / 4)
{
	//Seed the rand table.
	std::srand(time(NULL));
	...
	//Create the outline for the particle array to distinguish its space limits.
	...
	//Create the printer object. This object will be used for printing each particle
...
Simulator::~Simulator()
{
	//De allocate space used by particle array.
	for (int i = 0; i < width; i++)
		delete[] particles[i];
	delete[] particles;
...
{
	//Capture mouse input if mouse is within the outline of the particle array.
	...
	//Update particles.
	//FIND WAY TO RANDOMIZE WHETHER IT GOES LEFT OR RIGHT IF PRESENTED WITH
	//TWO OPTIONS OF EQUAL IMPORTANCE.
	for (int y = height - 1; y >= 0; y--)
		for (int x = width - 1; x >= 0; x--)
		{
			if (!particles[x][y].checked && particles[x][y].type != None)
			{
				...
				{
				case Sand:
					if (valid_index(x, y + 1) && swappable(particles[x][y].type, particles[x][y + 1].type) &&
					...
					//Impliments rand_1 to randomly determine whether left or right side is checked first.
					else if (valid_index(x - rand_1, y + 1) && swappable(particles[x][y].type, particles[x - rand_1][y + 1].type) &&
				...
				case Water:
					...
					else if (valid_index(x - rand_1, y) && swappable(particles[x][y].type, particles[x - rand_1][y].type) &&
	...
	//Set all particles to unchecked.
	for (int i = 0; i < width; i++)
		for (int j = 0; j < height; j++)
			particles[i][j].checked = false;
...
//change the type of particle that is at a specific location. if remove = true, use
//Type None instead of the selected type.
	...
	const bool remove)
{
	for(int x = -placement_size; x <= placement_size / 2; x++)
		...
		{
			if (xPos + x < width && xPos + x > -1 && yPos + y < height && yPos + y > -1)
			{
				if(remove)
...
void Simulator::keyboard_input()
{
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::LBracket) && !left_bracket_pressed)
	{
	...
	else if (!sf::Keyboard::isKeyPressed(sf::Keyboard::LBracket) && left_bracket_pressed)
		left_bracket_pressed = false;

	if (sf::Keyboard::isKeyPressed(sf::Keyboard::RBracket) && !right_bracket_pressed)
	{
	...
	else if (!sf::Keyboard::isKeyPressed(sf::Keyboard::RBracket) && right_bracket_pressed)
		right_bracket_pressed = false;
